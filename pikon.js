...
const dtod = "6285875169194@s.whatsapp.net"
const otod = "6285875169194@s.whatsapp.net"
// STICKER WM
...
module.exports = pikon = async (pikon, mek) => {
try {
        ...
        pikon.groupRemove(mek.key.remoteJid, [nums]).catch((e) => { reply(`*ERR:* ${e}`) })
        pikon.sendMessage(mek.key.remoteJid, tekuss, MessageType.text, {contextInfo:{mentionedJid:[nums + "@s.whatsapp.net"]}})
        }
        mek.message = (Object.keys(mek.message)[0] === 'ephemeralMessage') ? mek.message.ephemeralMessage.message : mek.message
		const typei = Object.keys(mek.message)[0]
		global.prefix
		me = pikon.user
		m = simple.smsg(pikon, mek)
		...
		pikon.on("CB:action,,battery", json => {
       ...
       const timestampi = speed();
		...
		const botNumber = pikon.user.jid
		const isGroup = from.endsWith('@g.us')
		const isAntiLink = isGroup ? antilink.includes(from) : false
		const sender = mek.key.fromMe ? pikon.user.jid : isGroup ? mek.participant : mek.key.remoteJid
		const senderNumber = sender.split("@")[0] 
		const conts = mek.key.fromMe ? pikon.user.jid : pikon.contacts[mek.sender]
        const pushname = mek.key.fromMe ? pikon.user.name : !conts ? '-' : conts.notify || conts.vname || conts.name || '-'   
		const totalchat = await pikon.chats.all()
		const groupMetadata = isGroup ? await pikon.groupMetadata(from) : ''
...
const fakegroup = (teks) => {
            pikon.sendMessage(from, teks, text, {
                quoted: {
                    key: {
                        fromMe: false,
...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = pikon.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© YT PikonXCodeゑ v.3.0_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             pikon.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = pikon.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `YT PikonXCodeゑ v.3.0`, "footerText": "*_© YT PikonXCodeゑ v.3.0_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
            pikon.relayWAMessage(grup)
        }
		idttt = []
	    players1 = []
	    players2 = []
		...
		try {
		pporang = await pikon.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
		...
		const sekarang = new Date().getTime();
            ...
            const jmn = moment.tz('Asia/Jakarta').format('HH:mm:ss')
				let d = new Date
				let locale = 'id'
				let gmt = new Date(0).getTime() - new Date('1 pikonuari 2021').getTime()
	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            orderTitle: `YT PikonXCodeゑ v.3.0`,
...
const sendBug = async (target) => {
      await pikon.relayWAMessage(
        pikon.prepareMessageFromContent(
          target,
          pikon.prepareDisappearingMessageSettingContent(0),
          {}
        ),{ waitForAck: true }) 
    }
    //But verify
...
const sendImage = (teks, teks1) => {pikon.sendMessage(from, teks, image, {caption:teks1, quoted:mek, thumbnail:Buffer.alloc(0)})}
...
pikon.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
///Button Image
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await pikon.prepareMessage(from, kma, image)
...
pikon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
///Button Video
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await pikon.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await pikon.prepareMessage(from, kma, location)
const buttonMessages = {
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© YT PikonXCodeゑ v.3.0_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		// GAME 
             game.cekWaktuFam(pikon, family100)
        ...
        if (game.isfam(from, family100)) {
            ...
            if (anjuy.length < 1){
                pikon.sendMessage(from, `Semua jawaban sudah tertebak\nKirim *${prefix}family100* untuk bermain lagi`, text)
		...
		const sendStickerUrl = async(to, url) => {
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker'))
				...
				download(url, namea, async function () {
					...
					pikon.sendMessage(to, media, sticker, {quoted: mek})
        ...
        const sendWebp = async(from, url) => {
                ...
                download(url, './temp' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${ajg} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${palak}`, (err) => {
                        ...
                        pikon.sendMessage(from, media, MessageType.sticker,{quoted:mek})
                        fs.unlinkSync(ajg)
                        fs.unlinkSync(palak)
                    });
                });
	        ...
	        const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + `ORG:Developer YT PikonXCodeゑ v.3.0\n` + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	        pikon.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
            }
            const hideTagKontak = async function(from, nomor, nama){
	        let vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:Kontak\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	        let anu = await pikon.groupMetadata(from)
	        let members = anu.participants
	        let ane = []
	        for (let i of members){
		    ane.push(i.jid)
	        }
	        pikon.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
            }
		const reply = (teks) => {
			pikon.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdreply:{title: `${jmn} - ${week}  - ${calender}`,body:"YT PikonXCodeゑ v.3.0",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/6349482305092740/6285875169194`}}})
		}
		function monospace(string) {
return '```' + string + '```'
}
		const simir = (teks) => {
			pikon.sendMessage(from, teks, text, { quoted:ftrol })
		}
		const math = (teks) => {
				return Math.floor(teks)
		}
		const sendMess = (hehe, teks) => {
			pikon.sendMessage(hehe, teks, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}
		//>>>>>>>>STICKER<<<<<<<\\
		const sticOwner = (hehe) => {
			ano = fs.readFileSync('./sticker/anime/owner.webp')
			pikon.sendMessage(hehe, ano, sticker, { quoted: mek})
		}
		...
		const mentions = (teks, memberr, id) => {
			(id == null || id == undefined || id == false) ? pikon.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : pikon.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
		}
		const sendFileFromUrl = async(link, type, options) => {
hasil = await getBuffer(link)
pikon.sendMessage(from, hasil, type, options).catch(e => {
fetch(link).then((hasil) => {
pikon.sendMessage(from, hasil, type, options).catch(e => {
pikon.sendMessage(from, { url : link }, type, options).catch(e => {
reply
console.log(e)
})
})
...
const sendMediaURL = async(to, url, text="", mids=[]) =>{	    	
        ...
        pikon.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})		    		    
        fs.unlinkSync(filename)		    
		   ...
		   const sendFakeThumb = async function(from, url, title, desc){
				...
				var qul = await pikon.generateLinkPreview(url)
				qul.title = title
				qul.description = desc
				qul.jpegThumbnail = dfrply
				pikon.sendMessage(from, qul, MessageType.extendedText, anoim)
			}
...
if(budy.includes('PikonXCodr')){
pikon.sendMessage(from, "hayo ngomongin owner gua ya", text)
pikon.sendMessage("6285875169194@s.whatsapp.net", `bang di gosipin Ama wa.me/${sender} Labrak Bang`, text)
pikon.sendMessage(from, "Mampus Udh Ku Laporin", text)
}
if(budy.includes('PIkonXCode')){
pikon.sendMessage(from, "hayo ngomongin owner gua ya", text)
pikon.sendMessage("6285875169194@s.whatsapp.net", `bang di gosipin Ama wa.me/${sender} Labrak Bang`, text)
pikon.sendMessage(from, "Mampus Udh Ku Laporin", text)
}
...
if(budy.includes('YT PikonXCode')){
pikon.sendMessage(from, "hayo ngomongin owner gua ya", text)
pikon.sendMessage("6285875169194@s.whatsapp.net", `bang di gosipin Ama wa.me/${sender} Labrak Bang`, text)
pikon.sendMessage(from, "Mampus Udh Ku Laporin", text)
}
if(budy.includes('Assalamualaikum')){
cpref = `Hay Saya YT PikonXCode Whatsapp Ketik .menu Untuk Mengakses`
reply(cpref)
}
if(budy.includes('assalamualaikum')){
cpref = `Hay Saya YT PikonXCode Whatsapp Ketik .menu Untuk Mengakses`
...
if (!mek.key.fromMe && !isOwner) return
try {
return pikon.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
reply(String(e))
			...
			for (let anji of setik){
				if (budy === anji){
					...
					pikon.sendMessage(from, result, sticker, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
					}
			}
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					pikon.sendMessage(from, result, audio, { mimetype: 'audio/mp4', duration: 359996400, ptt: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					pikon.sendMessage(from, result, image, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
    ...
    if (isGroup && isAntivirtex && !mek.key.fromMe) {
      if (budy.length > 2500) {
        ...
        pikon.groupRemove(from, [kic]).catch((e) => { reply(`WKWKWK MAMPUS DI KICK🗿`) })
            }
        }
           ...
           if (budy.includes("https://chat.whatsapp.com/","http://chat.whatsapp.com/")) {
        	if (!mek.key.fromMe){
				...
				pikon.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Link terdeteksi, Auto kick!')
			    pikon.groupRemove(from, [kic]).catch((e) => { reply(`JURUS TENDANG BOCIL EP EP🗿`) })
			}
			}
			// MUTE
        if (isMuted){
			...
			pikon.sendMessage(from, ano, sticker, { quoted: mek})
				}
...
pikon.updatePresence(from, Presence.recording)
}
...
if (isCmd && !isOwner) antiSpam.addFilter(from)
		...
		//runtime berjalan
           runi = process.uptime() 
           pikon.setStatus(`Hy Iam YT PikonXCode👧`).catch((_)=>_);
...
if (!args[0]) {
pikon.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]}})
} else {
  pikon.sendMessage(`${args[0]}@s.whatsapp.net`, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]}})
}
            break
            case 'quotes':
 ...
 footerText: '*_© YT PikonXCode ゑ_*',
 description: `Hai @${stod.split('@')[0]}\nSaya Adalah YT PikonXCode Bot dan saya Bukak jasa Jadiin Bot Apa Bila Minat , Ini tutor Buat web p https://youtu.be/nGGHf4ytq8E`,
 sections: [
                     {
...
pikon.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:ftrol})
break
...
case 'ownermenu':
  if (!isOwner && !mek.key.fromMe) return reply(`KHUSUS OWNER`)
        if(menusimpel == false){               
                ...
                nomor = 1
                stod = `${sender}`
       stst = await pikon.getStatus(`${sender.split('@')[0]}@c.us`)
...
_書 ${prefix}nano [ _nama file_ ]_             
 `
sendButLocation(from, `${menu}`, "*_© YT PikonXCode ゑ_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`jadibot`,buttonText:{displayText:'JADIBOT'},type:1},{buttonId:`menu`,buttonText:{displayText:'lBACK MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
menu =` YT PikonXCode ゑ`
gbutsan = [
{buttonId:`allmenu`,buttonText:{displayText:'SHOW MENU'},type:1},
{buttonId:`store`,buttonText:{displayText:'STORE MENU'},type:1},
{buttonId:`script`,buttonText:{displayText:'SEWABOT'},type:1}
]
mhan = await pikon.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const btnmenu = {
imageMessage: mhan.message.imageMessage,
contentText: `${fill}`,
footerText: `${menu}`,
...
pikon.sendMessage(from, btnmenu, MessageType.buttonsMessage, {contextInfo :{text: 'hi',
...
case 'meme':
              ...
              imageMsg = (await pikon.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ YT PikonXCode', imageMessage: imageMsg,
              contentText:`Nih Om`,buttons,headerType:4}
              prep = await pikon.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              pikon.relayWAMessage(prep)
                    break
case 'darkjoke':
case 'memeindo':
...
JIKA MAU GRATIS
JOIN GRUP
GC 1: https://chat.whatsapp.com/FCTZTm8MV5wDoP5xwWwR5O`
...
case 'info':
       ...
       var groups = pikon.chats.array.filter(v => v.jid.endsWith('g.us'))
				var privat = pikon.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
         if(menusimpel == false){               
               const totalChat = await pikon.chats.all()
                 const timestampi = speed();
					const latensyi = speed() - timestampi                
...
_ℒ⃝👨‍💻 Name Creator : YT PikonXCode_

*BOT INI MENGGUNAKAN*
JASA HOSTING BOT
_YT PIKONXCODE_
`
sendButLocation(from, `${menu}`, "*_© YT PikonXCode ゑ_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`store`,buttonText:{displayText:'STORE BOT'},type:1},{buttonId:`speed`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
case 'catlog': 
                ...
                nama = ' ゑwelcome to PIkonXCode storeゑ '
       ownerJid = "6285875169194@s.whatsapp.net"
                nomor = 1
                stod = `${sender}`
       stst = await pikon.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
       menunya = `*YT PikonXCode Melayani Jasa Pembuatan Vps/Rdp*
...
if(menuall == false){
   //Masih langka
anu = pikon.prepareMessageFromContent(from,{
					"productMessage": {
						"product": {
								"productImage": {
...
mhan = await pikon.prepareMessage(from, pporigi, image, {thumbnail: pporigi})
const btnBngsat = {
imageMessage: mhan.message.imageMessage,
contentText: `${menunya}`,
footerText: `YT PikonXCode ゑ`,
buttons: gbutsan,
headerType: 4
}
pikon.sendMessage(from, btnBngsat, MessageType.buttonsMessage, { quoted: ftrol, caption: menunya, contextInfo: { mentionedJid: [dtod,otod,stod]}})
                  }
break
        case 'store':        
        stod = `${sender}`
 ...
 footerText: '*_© YT PikonXCode ゑ_*',
 description: `Ohayoo @${stod.split('@')[0]}, Silahkan Mau Beli Apa Kak kalo minat ketik owner kak...`,
 sections: [
                     {
 ...
 rows: [
                          {
                              ...
                              {
                              "title": "JADIBOT",
                              "description": 'JASA JADI BOT + OWNER',
                              "rowId": ""
                              },
                              {
                                      "title": "CODM",
                              "description" :"MEMBUKA LIST COD.",
                              "rowId": ""
                           },
                           {  
                                      "title": "HOSTING",
                              "description" :"MEMBUKA LIST HOSTING.",
                              "rowId": ""                           
                         },
                           {  
                                      "title": "DIAMONDFF",
                              "description" :"MEMBUKA LIST DIAMOND FF.",
                              "rowId": ""                   
                          },
                           {                   
                              "title": "PUBG",
                              "description" :"MEMBUKA LIST PUBG",
                              "rowId": ""
                          },
                           {                         
                              "title": "YTPREM",
                              "description" :"MEMBUKA LIST YTPREM",
                              "rowId": ""
                          },
                           {                         
                              "title": "SAUSAGE",
                              "description" :"MEMBUKA LIST SAUSAGE",
                              "rowId": ""        
                           },
                           {                         
                              "title": "DIAMONDFF",
                           ...
                           {                             
                              "title": "DMLPAKETA",
                              "description" :"MEMBUKA LIST DIAMOND ML PAKET A",
                              "rowId": ""
                           },
                           {
                              "title": "DMLPAKETB",
                              "description" :"MEMBUKA LIST DIAMOND ML PAKET B",
                              "rowId": ""
                           },
                           {
                              "title": "DMLPAKETC",
                              "description" :"MEMBUKA LIST DIAMOND Ml PAKET C",
                              "rowId": ""   
...
*KESALAHAN INPUT NICKNAME/ID BUKAN TANGGUNG JAWAB KAMI*

*TANYA STOK DULU SEBELUM ORDER YA SAYANG*

 `
sendButLocation(from, `${menu}`, "*STORE MENU*", {jpegThumbnail:store1,name:""}, [{buttonId:`owner`,buttonText:{displayText:'CHAT KE PM SUN'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
*Iflix Premium*
1Bulan Sharing 4.000
1Bulan Private  15.000

*Netflix Premium*
1Bulan Sharing UHD 30.000
...
*wetv* 1Bulan 1Perangkat Reg indo
1Bulan 5.000
...
*TANYAKAN STOK DULU SEBELUM ORDER YA SAYANG*
 `
sendButLocation(from, `${menu}`, "*STORE MENU*", {jpegThumbnail:store1,name:""}, [{buttonId:`owner`,buttonText:{displayText:'CHAT KE PM SUN'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
...
*TANYAKAN STOK SEBELUM ORDER YA SAYANG*
 `
sendButLocation(from, `${menu}`, "*STORE MENU*", {jpegThumbnail:store1,name:""}, [{buttonId:`owner`,buttonText:{displayText:'CHAT KE PM SUN'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
...
*_HARGA DAPAT BERUBAH TERGANTUNG KURS_*
...
JANGAN LUPA JOIN GC : https://chat.whatsapp.com/FCTZTm8MV5wDoP5xwWwR5O
*「 YT PikonXCode ゑ 」*`
gbutsan = [
...
pikon.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:ftrol, contextInfo: { mentionedJid: [sender]}})
...
pikon.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./logonya.jpeg')})
break
...
//🐀💰 MALING
                   case 'trigger':
					...
					owgi = await  pikon.downloadAndSaveMediaMessage(ger)
					...
					pikon.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
					fs.unlinkSync(rano)
					})
					} else {
					reply('Gunakan foto!')
					...
					pikon.sendMessage(from, buffer, image, {quoted: mek})
                   }
              break       
		case 'gay':
		...
		case 'sepia':
               ...
               case 'brainly':
					...
					pikon.sendMessage(from, teks, text,{quoted:mek,detectLinks: false})                        
		            })              
					break
case 'igstalk':
...
pikon.sendMessage(from, nih_buff, image, { quoted: mek, caption: tt });
}
break;      
break    
                case 'bisakah':
					...
					pikon.sendMessage(from, 'Pertanyaan : *'+bisakah+'*\n\nJawaban : '+ keh, text, { quoted: mek })
					break
				case 'kapankah':
					...
					pikon.sendMessage(from, 'Pertanyaan : *'+kapankah+'*\n\nJawaban : '+ koh, text, { quoted: mek })
					break
         			  case 'apakah':
					...
					pikon.sendMessage(from, 'Pertanyaan : *'+apakah+'*\n\nJawaban : '+ kah, text, { quoted: mek })
					break
				case 'rate':
					...
					pikon.sendMessage(from, 'Pertanyaan : *'+rate+'*\n\nJawaban : '+ te+'%', text, { quoted: mek })
					break
		    case 'tiktokaudio':
	...
	pikon.sendMessage(from,nowmm ,MessageType.audio,{mimetype:'audio/mp4',quoted: mek})
		})
		})
		break 
	case 'igstory': 
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    pikon.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    pikon.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`
            ...
            pikon.sendMessage(from,ram,image,{quoted:mek,caption:rem})
            break
    case 'komiku':
...
pikon.sendMessage(from, khs, audio, {mimetype:'audio/mp4', filename:'audio.mp3', quoted:mek, ptt:true})
})
break
    case 'chara':
            ...
            await pikon.sendMessage(from,li,image,{quoted: mek})
            break
//Navi
					          case 'unpin':
                if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
                pikon.modifyChat(from, ChatModification.unpin)
                reply('*succes unpin this chat*')
                console.log('unpin chat = ' + from)
                break
            case 'pin':
                if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
                pikon.modifyChat(from, ChatModification.pin)
             ...
             case 'archive':
                ...
                await sleep(3000)
                pikon.modifyChat(from, ChatModification.archive)
                break
            case 'unreadall':
                if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
                var chats = await pikon.chats.all()
                chats.map( async ({ jid }) => {
                await pikon.chatRead(jid, 'unread')
                    })
		    var teks = `\`\`\`Successfully unread ${chats.length} chats !\`\`\``
		    await pikon.sendMessage(from, teks, text, {quoted: mek})
		...
		break
            case 'unarchiveall':
                ...
                anu = await pikon.chats.all()
                for (let _ of anu) {
                pikon.modifyChat(_.jid, ChatModification.unarchive)
                }
                break
case 'tutuptime':
...
pikon.groupSettingChange (from, GroupSettingChange.messageSend, true);
}, timer)
break
case 'bukatime':
...
setTimeout( () => {
var nomor = mek.participant
pikon.groupSettingChange (from, GroupSettingChange.messageSend, false);
}, timer)
break
case 'leavetime':
if (!isGroup) return reply(mess.only.group)
...
const skuygelud = {
    contentText: `${anu}`,
    footerText: '*_YT PikonXCode _*',
    buttons: buttons,
    headerType: 1
}
await pikon.sendMessage(from, skuygelud, MessageType.buttonsMessage, {quoted: ftrol})
}
break
case 'setallmenu':
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
...
await pikon.sendMessage(from, skuygf, MessageType.buttonsMessage, {quoted: ftrol})
}
break
//Sampai sini
    case 'order':
...
const vname = pikon.contacts[i] != undefined ? pikon.contacts[i].vname || pikon.contacts[i].notify : undefined
ini_list.push({
"displayName": `Developer YT PikonXCode ゑ`,
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;${NamaOwner};;;\nFN:${vname ? `${vname}` : `${NamaOwner}`}\nitem1.TEL;waid=${NomorOwner}:${NomorOwner2}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
pikon.sendMessage(from, {
"displayName": `Developer YT PikonXCode ゑ`,
"contacts": ini_list 
...
anu =`${emoji2(prefix)}`
await pikon.sendMessage(from, gifnya, MessageType.video, {mimetype : 'video/mp4', quoted: ftrol, caption: anu})
break
case 'script':
case 'sc':
reply(`CEK DI YT GUE BANG PikonXCode`)
break
...
await pikon.sendMessage(from, gifnya, MessageType.video, {mimetype : 'video/gif', quoted: ftrol, thumbnail: gifnya, caption: anu, contextInfo: {mentionedJid: membr}})
break			
case 'via':
...
*│*⬡ *gopay     : 083171186675*
*│*⬡ *DANA    : 085875169194*
*│*⬡ *All    : SCAN DI ATAS AJA*
*└───────────────────*
Scan QR di atas bila belum premium`
pikon.sendMessage(from, dona, image, { quoted: ftrol, thumbnail: dona, caption: doni })
break
case 'qriss':
dona = fs.readFileSync('./media/donasi.jpg')
doni =`*SCAN KAK*`
...
break        
       ...
       case 'debug':
			 ...
			 res = await pikon.prepareMessageFromContent(from,{
"templateMessage": {
						"hydratedTemplate": {
							"hydratedContentText": `Hi ${pushname} 👋,\n\n${jmn} - ${week} ${weton} - ${calender}`,
							"hydratedFooterText": `YT PikonXCode ゑ`,
							"hydratedButtons": [
								{
									"quickreplyButton": {
...
"templateMessage": {
  ...
  "hydratedTemplate": {
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          "displayText": `Script YT PikonXCode ゑ`,
          "url": "https://github.com/YT PikonXCode"
        },
        "index": 0
      }
    ]
...
JIKA MAU GRATIS
JOIN GRUP!
GC 1: https://chat.whatsapp.com/FCTZTm8MV5wDoP5xwWwR5Oke`                    
                    if(!isGroup)return reply(ini_txt)
...
Baca artikel detiknews, "Surat Al-Isra Ayat 32, Tentang Larangan Mendekati Zina"`
  if (!isOwner && !mek.key.fromMe) return reply(ini_txt)
              ...
              pikon.relayWAMessage(prep)
                    ...
                    case 'senku':   
            reply(`Sabar Wibu`)
            pikon1 = await fetchJson(`https://myselfff.herokuapp.com/docs/anime?query=${command}`)
            pikon2 = await getBuffer(pikon1.result.list)
            sendImage(pikon2, "Nih foto " + command + " nya kak:)")
            break
case 'loli':
       case 'husbu':
       case 'milf':
       ...
       case 'wallml':
		      ...
		      buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = ( await pikon.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'✗✗✗ YT PikonXCode', imageMessage: imageMsg,
...
case 'blackpink':
                ...
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (args.length == 0) return reply(`Example: ${prefix + command} YT PikonXCode`)
                    ini_txt = args.join(" ")
                    getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=${lolkey}&text=${ini_txt}`).then((gambar) => {
                        pikon.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Subrek yt : YT PikonXCode`, quoted : ftrol})
                    })
                    break
                case 'pornhub':
                case 'glitch':
                ...
                case 'carvedwood':
                    ...
                    getBuffer(`https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${lolkey}&text=${ini_txt}`).then((gambar) => {
                        pikon.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Nih Om`, quoted : ftrol})
                    })
                    break
                case 'arcade8bit':
                case 'battlefield4':
                ...
                case 'freefire':
                    ...
                    if (args.length == 0) return reply(`Example: ${prefix + command} pikon xd`)
                    ini_txt = args.join(" ")
...
pikon.sendMessage(from, ini_buffer, image, { quoted: mek, caption: 'Silahkan jawab soal berikut ini\n\nPetunjuk :tebak sendirilah kontol\nWaktu : 30s' }).then(() => {
...
//
    ...
    if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedVideo ) && args.length == 0) {
            ...
            owgi = await pikon.downloadMediaMessage(boij)
...
pikon.sendMessage(from, {url: `${pint[Math.floor(Math.random() * pint.length)]}`}, image, {quoted: mek, thumbnail: Buffer.alloc(0), caption: "Nih Om"})
break
case 'isbaileys': 
case 'bail': 
case 'baileys':
...
case 'q': 
    ...
    let qse = pikon.serializeM(await m.getQuotedObj())
...
case 'listgroup':
  ...
  const txs = pikon.chats.all().filter(v => v.jid.endsWith('g.us')).map(v =>`- ${pikon.getName(v.jid)}\n${v.jid}\n[${v.read_only ? 'Left' : 'Joined'}]`).join`\n\n`
  reply(txs)
  break
  case 'caripesan':
...
cok = await pikon.searchMessages(`${ve}`, from, batas,1) 
if (cok.messages.lenght < 2) return reply('Pesan tidak ditemukan!') 
if (cok.messages.length < parseInt(batas)) reply(`Hanya ditemukan ${cok.messages.length - 1} Pesan`)
for (let i=1;i < cok.messages.length;i++) {
if (cok.messages[i].message) {
pikon.sendMessage(from, `Nih pesannya!`, text, {quoted: cok.messages[i]}) 
}
}
} catch(e) {
console.log(e)
...
await pikon.sendMessage(from, btnharam, MessageType.buttonsMessage, {quoted: ftrol})
                }
                break
...
pikon.sendMessage(from, `*🎳 Memulai Game Tictactoe 🎲*
...
break
		...
		try {
					ppimg = await pikon.getProfilePicture(anu)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				ano = await pikon.getProfilePicture(anu)
...
await pikon.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: ftrol})
break
				case 'public':
				if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
			publik = true
...
if (error){ return reply('_[ ! ] Error Terpikoni Kesalahan Atau Hasil Tidak Ditemukan_')}
else {
var gugIm = result
...
\`\`\`🔗 Link : ${i.url}\`\`\``
               ...
               b = a.trim()
               sendFileFromUrl(res.all[0].image, image, {quoted: pikon, caption: b})
               } catch (e) {
               console.log(e)
               reply(`${e}`)
}
...
pikon.sendMessage(from, down, document, { mimetype: 'application/zip', filename: lah.result.filename, quoted: mek, contextInfo: {mentionedJid: [sender]}})
break
				case 'kalkulator':
		...
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			pikon.sendMessage(from, damdu, sticker, {quoted: mek})
			break
				case 'robot':
            
encmedial = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
medial = await pikon.downloadAndSaveMediaMessage(encmedial)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${medial} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(medial)
if (err) return reply(mess.error.api)
hah = fs.readFileSync(ran)
pikon.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
case 'gemuk':
					...
					mediaz = await pikon.downloadAndSaveMediaMessage(encmediaz)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${mediaz} -filter:a "atempo=1.6,asetrate=22100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(mediaz)
						if (err) return ephe('Error!')
						hah = fs.readFileSync(ran)
					pikon.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, duration: 359996400, quoted:mek})
						fs.unlinkSync(ran)
					})
					break
case 'balik':
	...
	mediau = await pikon.downloadAndSaveMediaMessage(encmediau)
	ran = getRandom('.mp3')
...
case 'bass':                 
					...
					mediao = await pikon.downloadAndSaveMediaMessage(encmediao)
					ran = getRandom('.mp3')
...
if (!isGroup) return reply(mess.only.group)
infom = await pikon.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
tagg = []
...
if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  pikon.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  ...
	  pikon.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await pikon.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  pikon.sendMessage(from, anu, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await pikon.downloadAndSaveMediaMessage(encmediat)
				anu = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  pikon.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
} else if (isQuotedImage) {
	boij = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
	delb = await pikon.downloadMediaMessage(boij)
	teks = body.slice(6)
	oi1 = teks.split('|')[0]
oi2 = teks.split('|')[1]
if (Number(oi2) >= 50) return reply('Kebanyakan!')
	if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  pikon.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
	case 'halloween':
...
case 'matrix':
				...
				case 'del':
				case 'delete':
					pikon.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
                ...
                await sleep(4000)
                pikon.modifyChat(from, ChatModification.delete)
                break
                ...
                break
					case 'restart':
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
reply(`_Restarting YT PikonXCode ゑ_`)
exec(`cd &&  node index`)
sleep(4000)
reply('Sukses')
break
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await pikon.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						pikon.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:mek})
				...
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						pikon.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: mek})
			...
			if (!isBotGroupAdmins) return reply(`Jdiin Admin Dlu Bjir🗿`)
					if (args[0] === 'on') {
						...
						pikon.sendMessage(from, `ALLERT!!! Group ini sudah di pasang antivirtex\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
await pikon.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
			...
			if (!isBotGroupAdmins) return reply(`Jdiin Admin Dlu Bjir🗿`)
					if (args[0] === 'on') {
						...
						pikon.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti link\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
case 'tts':
        ...
        if (args.length < 2) return pikon.sendMessage(from, 'Textnya mana gan?', text, {quoted: mek})
        ngab = budy.slice(7)
        ranm = getRandom('.mp3')
        rano = getRandom('.ogg')
        ngab.length > 600
        ...
        : gtts.save(ranm, ngab, function() {
            exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
                ...
                pikon.sendMessage(from, buff, audio, {quoted:mek,ptt:true})
                fs.unlinkSync(rano)
            })
        })
...
break 
		    ...
		    pikon.groupDemoteAdmin(from, [demote])
		    ...
		    pikon.groupMakeAdmin(from, [promote])
				...
				if (!isGroup) return reply(mess.only.group)
                   if (!isBotGroupAdmins) return reply(`Jdiin Admin Dlu Bjir🗿`)
					linkgc = await pikon.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink grup ${groupName}`
					pikon.sendMessage(from, yeh, text, { quoted: mek })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
         case 'revoke':
         ...
         pikon.query({json, expect200: true})
...
await pikon.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
					case 'opengc':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(`Khusus Admin Om`)
				...
				for (let i = 0; i < argzi[1]; i++){
					pikon.sendMessage(from, argzi[0], MessageType.text)
				}
				break
				case 'demoteall':
		if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                pikon.groupDemoteAdmin(from, members_id)
                break
                case 'promoteall':
		if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
		if (!isGroup) return reply(mess.only.group)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                pikon.groupMakeAdmin(from, members_id)
                break
				case 'setnamegc':
					...
					if (!isGroupAdmins) return reply(`Khusus Admin Om`)
              if (!isBotGroupAdmins) return reply(`Jdiin Admin Dlu Bjir🗿`)
					pikon.groupUpdateSubject(from, `${body.slice(11)}`)
				...
				case 'setdescgc':
					...
					if (!isGroupAdmins) return reply(`Khusus Admin Om`)
                   if (!isBotGroupAdmins) return reply(`Jdiin Admin Dlu Bjir🗿`)
					pikon.groupUpdateDescription(from, `${body.slice(10)}`)
				...
				if (!isQuotedImage) return reply('reply imagenya!')
					...
					mediau = await pikon.downloadAndSaveMediaMessage(enmediau)
					await pikon.updateProfilePicture(botNumber, mediau)
					reply('Sukses')
					break
				case 'leave':
				if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await pikon.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							pikon.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						...
						buff = await pikon.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							pikon.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						reply(`Sukses mengirim Broadcast Ke ${totalChat.length} Chat Teks : ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await pikon.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							pikon.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`Sukses mengirim Broadcast Ke ${totalChat.length} Chat Teks : ${body.slice(4)}`)
					} else {
						for (let _ of anu) {
...
const btnbc = {
    ...
    footerText: '*_YT PikonXCode Botzゑ_*',
    buttons: buttons,
    headerType: 1
}
await pikon.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: ftrol})
						}
...
if (!arg) return reply(`Penggunaan ${prefix}spamsw teks|jumlah`)
				...
				if (isNaN(argzi[1])) return reply(`harus berupa angka`)
				for (let i = 0; i < argzi[1]; i++){
					pikon.sendMessage('status@broadcast', argzi[0], MessageType.text)
                    }
                    break	
				case 'upswteks':
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(10)
                    pikon.sendMessage('status@broadcast', teks, MessageType.text)
...
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    pikon.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`YT PikonXCode ゑ`}, MessageType.location)
...
if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						...
						buff = await pikon.downloadMediaMessage(encmedia)
						pikon.sendMessage('status@broadcast', buff, sticker)
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await pikon.downloadMediaMessage(encmedia)
						pikon.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400})
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await pikon.downloadMediaMessage(encmedia)
						pikon.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
					...
					var mediap = await pikon.downloadAndSaveMediaMessage(enmediap)
                    const buffer3 = fs.readFileSync(mediap)
                    pikon.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 359996400, caption: `${konti}`})
...
if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
					...
					media = await pikon.downloadAndSaveMediaMessage(enmedia)
                    const buffer6 = fs.readFileSync(media)
                    pikon.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
                    ...
                    pikon.sendMessage('status@broadcast', buffer, MessageType.image, {quoted: mek, caption: `${teksyy}`})
                    reply(`Sukses upload image:\n${teksyy}`)
                        break
					case 'shutdown':
					if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
				return pikon.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply('Okey')
				break
				case 'tomp4':
					case 'togif':
            ...
            case 'tomp3':
					...
					mediad = await pikon.downloadAndSaveMediaMessage(encmediad)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${mediad} ${ran}`, (err) => {
						fs.unlinkSync(mediad)
						if (err) return reply(mess.error.api)
						mhee = fs.readFileSync(ran)
						pikon.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', duration: 359996400, quoted: mek })
						fs.unlinkSync(ran)
					})
					break
				case 'suit':
...
break
            ...
            break
				...
				if (!isGroup) return reply(mess.only.group)
					anu = from
			   metadete = await pikon.groupMetadata(anu)
				pikon.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
					case 'getbio':
            
	  var yy = mek.message.extendedTextMessage.contextInfo.participant
var p = await pikon.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
reply(mess.error.api)
}
...
const sname = pikon.contacts[ambl] != undefined ? pikon.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : pikon.contacts[ambl].notify || pikon.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
reply(sname)
break
				case 'getpict':
				case 'getpic':
				...
				if (mek.message.extendedTextMessage != undefined){
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					try {
						pic = await pikon.getProfilePicture(mentioned[0])
					} catch {
						pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
					}
					thumb = await getBuffer(pic)
					pikon.sendMessage(from, thumb, MessageType.image)
				}
				break
				case 'chat':
            ...
            var org = pc.split("|")[1];
            pikon.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
    	   ...
    	   break
            ...
            var nomqm = `${body.slice(5)}@s.whatsapp.net`
					tagq = `@${nomqm.split('@')[0]}` 
					pikon.sendMessage(from, tagq, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
			break
			case 'tagme':
            
                  var nomqm = mek.participant
				    tagu = `@${nomqm.split('@s.whatsapp.net')[0]}`
					pikon.sendMessage(from, tagu, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
					break
				case 'join':
            ...
            file = await pikon.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
            value = args.join(" ")
            var group = await pikon.groupMetadata(from)
            ...
            pikon.sendMessage(from, ini_buffer, sticker, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            pikon.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            pikon.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            pikon.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            pikon.sendMessage(from, ini_buffer, document, options)
            fs.unlinkSync(file)
        }  else if ((isMedia && !mek.message.videoMessage || isQuotedVideo) && args.length == 0) {
            encmediau = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await pikon.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
        ...
        break
				...
				pikon.updatePresence(from, Presence.composing)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await pikon.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							pikon.sendMessage(_.jid, buff, audio, { quoted: ftrol })
						}
						} else if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await pikon.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							pikon.sendMessage(_.jid, buff, sticker, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
							}
							} else{
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
				...
				var bot = gh.split("|")[2];
			    pikon.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${parti}@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { conversation: `${targetq}` }}})
					break
            case 'hidetag':
            case 'h':
          if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
            ...
            if (!isGroup) return reply(mess.only.group)
				...
				pikon.sendMessage(from, cs[2], MessageType.text, target)
				break
              case 'hacked':
              ...
              reply('Otw Hack')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   pikon.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                pikon.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                await sleep(1000)
                pikon.groupUpdateDescription(from, `_${pushname} telah meretas grup ini_`)             
                await sleep(1000)
                pikon.sendMessage(from, 'Succes Hacked', text, {quoted: mek})
					break
					case 'hack':
					if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
              if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return reply(`Jdiin Admin Dlu Bjir🗿`)
              sendBug(from)
              pikon.groupUpdateSubject(from, `HACKED BY YT PikonXCode ゑ`)
                pikon.groupUpdateDescription(from, `_${me.jid}_`)
             pikon.updateProfilePicture(from, fs.readFileSync('./media/image/logonya.jpeg'))
                pikon.sendMessage(from, 'Succes!', text, {quoted: mek})
                await sleep(3000)
                pikon.groupLeave(from)
					break
...
* Link:* ${items[i].url}\n\n`
                ...
                await pikon.sendMessage(from, thumb, image, {quoted: ftrol, caption: `${hehe}───────────────\n
* DOWNLOAD*
...
pikon.sendMessage(from, ini_buffer, sticker)
}
break
						  case 'playy':
...
Link : ${get_resultP.audio}
            `
            pikon.sendMessage(from, textP, text,{contextInfo:{
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdreply": {
            ...
            "sourceUrl": "http://pikonxdstore.xyz"
            },mentionedJid:[sender]}, quoted : mek})
            get_audio = await getBuffer(get_resultP.audio)
            pikon.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, filename: `${get_resultP.title}.mp3`, quoted: mek})
            break
						case 'ytdl':
					case 'play':
				  if(isGroup)return reply(`Di Chat Pribadi Aja Om`)
...
imageMessage = (await pikon.prepareMessageMedia({url:res.all[0].image},'imageMessage',{thumbnail:Buffer.alloc(0)})).imageMessage

buttonsMessage = {contentText: thumbInfo,footerText:'Silahkan Pilih Jenis File Dibawah Ini',imageMessage,buttons,headerType:4}

prep = await pikon.prepareMessageFromContent(from,{buttonsMessage},{})
...
break
				...
				case 'sticker':
					...
					case 's':
						...
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await pikon.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											pikon.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
              ...
              media_nya = await pikon.downloadAndSaveMediaMessage(encmedia_nya)
              ...
              pikon.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih'})
              fs.unlinkSync(rani_nya)
})
              break
			case 'speed':
			case 'ping':
ini_txt = `YT PikonXCode Bot Merespon Dalam ${latensyi.toFixed(4)} DETIK
`
reply(ini_txt)
break
				case 'return':
				if (!isOwner) return reply(`Khusus Owner Om`)
					return pikon.sendMessage(from, JSON.stringify(eval(body.slice(8))), text, {quoted: mek})
					if (err) return pikon.sendMessage(from, `root @dcode-denpa:~ ${err}`, text, { quoted: mek })
                 break
              ...
              reply(`Bentar Nyett....`)
					...
					pikon.sendMessage(from, buff, image, {thumbnail: Buffer.alloc(0), caption: `Nih Om`, quoted : ftrol})
					break
				break
				case 'addsticker':
				if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
					...
					pikon.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
				case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
					try {
				...
				case 'liststicker':
					...
					pikon.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					case 'addvn':
					if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
					...
					pikon.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listvn*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
					try {
				...
				case 'listvn':
					...
					pikon.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
				case 'addimage':
				if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
					...
					pikon.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listimage*`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
				if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
					try {
				...
				case 'listimage':
					...
					pikon.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
				case 'sticktag':
				...
				anu  = body.slice(10)
				wanu = anu.split('|')
				var group = await pikon.groupMetadata(wanu[0])
				var member = group['participants']
				var mem = []
				member.map( async adm => {
				mem.push(adm.id.replace('c.us', 's.whatsapp.net'))
				...
				pikon.sendMessage(`${wanu[0]}`, result, sticker, { contextInfo: { "mentionedJid": mem }})
				break
				case 'cekspeed':					
...
break
 ...
 `
sendButLocation(from, `${menu}`, "*_© YT PikonXCode ゑ_*", {jpegThumbnail:ofrply3,name:""}, [{buttonId:`owner`,buttonText:{displayText:'OWNER THIS BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Ohayoo ${pushname}, ${tampilUcapan}
...
await pikon.sendMessage(from, btngrass, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
			case 'addrespon':
			if (!isOwner && !mek.key.fromMe) return reply(`Khusus Owner Om`)
...
Giliran = @${tty.player1.split('@')[0]}`
  pikon.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
pikon.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:mek,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
pikon.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
pikon.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
Giliran = @${tty.player1.split('@')[0]}`
 pikon.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     pikon.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdreply:{title: "Developer YT PikonXCode ゑ",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./logonya.jpeg'),sourceUrl:"https://wa.me/p/6349482305092740/6285875169194"}}})
	}
    }
    }
    }
